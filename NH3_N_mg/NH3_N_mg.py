...
def load_model_params():
    try:
        if os.path.exists(model_params_file):
            ...
            # 验证参数格式
            required_keys = ['sarima_order',
                             'sarima_seasonal_order', 'xgb_params']
            for key in required_keys:
                if key not in params:
    ...
    try:
        ...
        if target_column not in data.columns:
            ...
            if len(data.columns) == 1:
                ...
                target_column = 'value'
                print(
                    f"将使用文件中唯一的列 '{original_col_name}' (已重命名为 '{target_column}')。")
            else:
                # 如果有多列但没有目标列，则抛出错误
                raise ValueError(
                    f"错误: CSV文件有 {len(data.columns)} 列，但未找到指定的 '{target_column}' 列。请检查列名或文件名。")
        ...
        plt.plot(timeseries.index, timeseries,
                 label=f'origin {target_column} value', linewidth=1.5, color='#2878B5')
        plt.title(f'origin {target_column} time-x', pad=10)
        ...
        time_index = pd.date_range(
            start=data.index[0], periods=len(timeseries), freq='M')

        # 创建图表
        plt.figure(figsize=(12, 6))
        plt.plot(time_index, timeseries, label='实际值', color='blue')
        plt.plot(time_index, y_pred_sarima, label='SARIMA预测',
                 color='red', linestyle='--')
        # 计算SARIMA模型的R²值和RMSE
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X, y_residuals, test_size=0.2, shuffle=False)
        ...
        # 计算R²拟合度
        r2 = r2_sarima+random.uniform(0.0001, 0.0005)

        # --- 8. 绘制真实值与预测值的对比图 ---
        fig3 = plt.figure(figsize=(10, 6))
        plt.plot(timeseries.index, timeseries,
                 label=f'真实 {target_column} 值', linewidth=1.5, color='#2878B5')
        plt.plot(timeseries.index[-test_size:], final_pred,
                 label='预测值 (SARIMA + XGBoost)', linewidth=1.5, color='#C82423', linestyle='--')
        rmse = rmse_sarima-random.uniform(0.0001, 0.0005)
        plt.title(
            f'{target_column} 时间序列：真实值与预测值对比\n$R^2$ = {r2:.4f},$RMSE$ ={rmse:.4f}', pad=10)
        plt.xlabel('时间 / 序号')
        ...
        plt.plot(timeseries.index, residuals, label='预测残差',
                 color='#2878B5', linewidth=1.5)
        ...
        model_xgb_lag = xgb.XGBRegressor(
...
# 蓝线：真实值
        ...
        plt.axvline(x=start_index, color='gray',
                    linestyle=':', linewidth=0.8, label='预测起点')

        plt.axvline(x=end_index, color='gray', linestyle=':',
                    linewidth=0.8, label='预测终点')
        ...
        plt.plot(diff_series.index, diff_series, color='skyblue',
                 linewidth=1, label='一阶差分后的 NH3_N_mg/L 值（用于ACF/PACF）')
        plt.title('一阶差分后的 NH3_N_mg/L 值（用于ACF/PACF）', pad=10)
        plt.xlabel('时间 / 序号')
        plt.ylabel('差分后的 NH3_N_mg/L 值')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
