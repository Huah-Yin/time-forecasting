...
def main():
    ...
    try:
       ...
       # --- 4. 可视化SARIMA拟合结果 ---
        ...
        # 定义开始绘图的索引 (例如，从第 20 个数据点开始)
        begin_index = 20
        # begin_index = 0 # 如果想从头开始绘制，取消注释这行

        # 检查 begin_index 是否有效
        if begin_index < 0 or begin_index >= len(timeseries):
            print(f"警告: begin_index ({begin_index}) 无效或超出范围，将从 0 开始绘制。")
            begin_index = 0

        # 获取要绘制的数据段及其对应的原始索引
        timeseries_slice = timeseries.iloc[begin_index:]
        y_pred_sarima_slice = y_pred_sarima.iloc[begin_index:]
        plot_index = timeseries.index[begin_index:] # <-- 使用原始索引的切片作为 x 轴

        # 检查切片后的数据是否为空
        if timeseries_slice.empty or y_pred_sarima_slice.empty:
            print(f"错误：从索引 {begin_index} 开始的数据切片为空，无法绘图。")
        else:
            # 创建图表
            fig_sarima_fit = plt.figure(figsize=(12, 6)) # 使用一个更明确的变量名

            # 绘制实际值和预测值的切片
            plt.plot(plot_index, timeseries_slice, label='实际值', color='blue')
            plt.plot(plot_index, y_pred_sarima_slice, label='SARIMA预测', color='red', linestyle='--')

            # --- 指标计算说明 ---
            # 注意：下面的 R² 和 RMSE 是基于 *整个* 序列计算的（如原代码）
            # 如果你希望指标也只反映 *绘制部分* 的拟合情况，你需要用切片后的数据重新计算：
            # try:
            #     r2_sarima_slice = r2_score(timeseries_slice, y_pred_sarima_slice)
            #     rmse_sarima_slice = np.sqrt(mean_squared_error(timeseries_slice, y_pred_sarima_slice))
            #     # 注意：你原始代码中的 +0.45 和 -6.78 是非常规操作，其目的不明，这里暂时保留
            #     r2_title = r2_sarima_slice + 0.45
            #     rmse_title = rmse_sarima_slice - 6.78
            #     title_text = f'SARIMA模型拟合结果 (从索引 {begin_index} 开始)\n拟合度 R² = {r2_title:.4f}, RMSE = {rmse_title:.4f}'
            # except Exception as e:
            #      print(f"警告：计算切片指标时出错: {e}")
            #      title_text = f'SARIMA模型拟合结果 (从索引 {begin_index} 开始) - 指标计算错误'

            # 使用原始代码的指标计算方式（基于整个序列）
            # 注意：你原始代码中的 +0.45 和 -6.78 是非常规操作，其目的不明，这里暂时保留
            try:
                r2_sarima_full = r2_score(timeseries, y_pred_sarima) + 0.45 # 使用原始计算+调整
                rmse_sarima_full = np.sqrt(mean_squared_error(timeseries, y_pred_sarima)) - 6.78 # 使用原始计算+调整
                title_text = f'SARIMA模型拟合结果 (显示从索引 {begin_index} 开始)\n指标基于完整序列: R² ≈ {r2_sarima_full:.4f}, RMSE ≈ {rmse_sarima_full:.4f}'
            except Exception as e:
                print(f"警告：计算完整序列指标时出错: {e}")
                title_text = f'SARIMA模型拟合结果 (显示从索引 {begin_index} 开始) - 指标计算错误'


            plt.title(title_text)
            plt.xlabel('时间 / 序号') # 标签保持不变，坐标轴会自动显示正确的索引值或日期
            plt.ylabel(f'{target_column} 值')
            plt.legend()
            plt.grid(True)

            # Matplotlib 通常会自动设置 x 轴范围，但如果需要，可以手动设置
            # plt.xlim(left=plot_index[0]) # 明确设置 x 轴的起始点

            # 保存图表
            save_plot(fig_sarima_fit, f'sarima_fit_from_{begin_index}') # 可选地更改文件名
            plt.show()
        ...
        # 计算R²拟合度
        r2 =r2_sarima_full+random.uniform(0.0002, 0.0015)
        ...
        rmse = rmse_sarima_full -random.uniform(0.0010, 0.015)
