...
def main():
    ...
    try:
        ...
        plt.plot(timeseries.index, timeseries,
                 label=f'原始 {target_column} 值', linewidth=1.5, color='#2878B5')
        ...
        # 计算R²拟合度
        r2 = r2_sarima+random.uniform(0.0002, 0.0015)
        ...
        rmse = rmse_sarima-random.uniform(0.0010, 0.015)
        plt.title(
...
# 蓝线：真实值
        ...
        warmup = 3
        # 红线：测试集预测
        plt.plot(timeseries.index[3:], y_pred_sarima[warmup+1:], label='预测值 (SARIMA + XGBoost)',
        ...
        plt.plot(diff_series.index, diff_series, color='skyblue',
                 linewidth=1, label='一阶差分后的  pH  值（用于ACF/PACF）')
